...
const CaseStudyCard = ({ content }: CaseStudyCardProps) => {
  ...
  // path가 외부 URL인지 확인 (http:// 또는 https://로 시작)
  const isExternalUrl = content.path.startsWith('http://') || content.path.startsWith('https://');
  const href = isExternalUrl ? content.path : `/${content.path}`;
  ...
  const cardContent = (
    ...
    >
        ...
        <span className="font-plus-jakarta-sans text-2xl font-medium text-neutral-800 transition-colors duration-300 group-hover:text-blue-600 sm:text-3xl lg:text-5xl">
          {content.title}
        </span>
      </div>
    </div>
  ...
  // 외부 URL이면 <a> 태그 사용 (새 탭에서 열림), 내부 경로면 Next.js Link 사용
  if (isExternalUrl) {
    return (
      <a 
        href={href} 
