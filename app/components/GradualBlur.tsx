...
const GradualBlur: React.FC<PropsWithChildren<GradualBlurProps>> = (props) => {
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    if (isVertical) {
      ...
      baseStyle.width = (responsiveWidth || '100%') as string;
      baseStyle[config.position] = 0;
      baseStyle.left = 0;
      baseStyle.right = 0;
    } else if (isHorizontal) {
      baseStyle.width = (responsiveWidth || responsiveHeight) as string | undefined;
...
const injectStyles = () => {
  ...
  styleElement.textContent = `.gradual-blur{pointer-events:none;transition:opacity 0.3s ease-out}.gradual-blur-inner{pointer-events:none}.gradual-blur-inner>div{-webkit-backdrop-filter:inherit;backdrop-filter:inherit}.gradual-blur{isolation:isolate}@supports not (backdrop-filter:blur(1px)){.gradual-blur-inner>div{background:rgba(0,0,0,0.3);opacity:0.5}}`;
